...
var _ = Describe("replace_failed_process_groups", func() {
	...
	When("replacing pods on tainted nodes", func() {
		...
		When("a single process group is running on a tainted node", func() {
			...
			When(
				"the process group has the NodeTaintDetected condition but not the NodeTaintReplacing",
				func() {
					BeforeEach(func() {
						targetProcessGroup = cluster.Status.ProcessGroups[0]
						targetProcessGroup.UpdateCondition(fdbv1beta2.NodeTaintDetected, true)
						targetProcessGroup.UpdateConditionTime(
					...
					It("should not replace the process group", func() {
						...
						Expect(
							targetProcessGroup.ProcessGroupConditions[0].ProcessGroupConditionType,
						).To(Equal(fdbv1beta2.NodeTaintDetected))

						Expect(
							replaceFailedProcessGroups{}.reconcile(
								ctx.TODO(),
								clusterReconciler,
								cluster,
								nil,
			...
			When(
				"the process group has both the NodeTaintDetected condition and the NodeTaintReplacing condition",
				func() {
					BeforeEach(func() {
						targetProcessGroup = cluster.Status.ProcessGroups[0]
					...
					It("should replace the process group", func() {
						...
						Expect(
							targetProcessGroup.ProcessGroupConditions[1].ProcessGroupConditionType,
						).To(Equal(fdbv1beta2.NodeTaintReplacing))
						...
						Expect(
							getRemovedProcessGroupIDs(cluster),
						).To(ConsistOf(targetProcessGroup.ProcessGroupID))
					})
				},
			)

			When(
				"the process group has both the NodeTaintDetected condition and the NodeTaintReplacing condition but the node taint feature is disabled",
				func() {
					BeforeEach(func() {
						targetProcessGroup = cluster.Status.ProcessGroups[0]
						timestamp := time.Now().Add(-10 * time.Minute).Unix()
		...
		When("multiple process groups are running on tainted nodes", func() {
			...
			When("only one fault domain with tainted nodes is allowed", func() {
				It("shouldn't remove any pods on tainted nodes", func() {
					...
					Expect(
						cluster.Status.ProcessGroups[0].ProcessGroupConditions[0].ProcessGroupConditionType,
					).To(Equal(fdbv1beta2.NodeTaintDetected))
					Expect(
						cluster.Status.ProcessGroups[0].ProcessGroupConditions[1].ProcessGroupConditionType,
					).To(Equal(fdbv1beta2.NodeTaintReplacing))
					Expect(cluster.Status.ProcessGroups[1].ProcessGroupConditions).To(HaveLen(2))
					Expect(
						cluster.Status.ProcessGroups[1].ProcessGroupConditions[0].ProcessGroupConditionType,
					).To(Equal(fdbv1beta2.NodeTaintDetected))
					Expect(
						cluster.Status.ProcessGroups[1].ProcessGroupConditions[1].ProcessGroupConditionType,
					).To(Equal(fdbv1beta2.NodeTaintReplacing))
	...
	When("replacing failed process groups", func() {
		JustBeforeEach(func() {
			...
			result = replaceFailedProcessGroups{}.reconcile(
				ctx.Background(),
				clusterReconciler,
				cluster,
				nil,
		...
		Context("with no missing processes", func() {
			...
			It("should not mark anything for removal", func() {
				Expect(
					getRemovedProcessGroupIDs(cluster),
				).To(ConsistOf([]fdbv1beta2.ProcessGroupID{}))
			})
		})
		...
		When("fault domain replacements are disabled", func() {
			...
			Context("with a process that has been missing for a long time", func() {
				...
				Context("with no other removals", func() {
					It("should requeue", func() {
						...
						Expect(
							result.message,
						).To(Equal("Removals have been updated in the cluster status"))
					})
				...
				Context("with multiple failed processes", func() {
					BeforeEach(func() {
						secondProcessGroup.ProcessGroupConditions = append(
							secondProcessGroup.ProcessGroupConditions,
							&fdbv1beta2.ProcessGroupCondition{
								ProcessGroupConditionType: fdbv1beta2.MissingProcesses,
								Timestamp:                 time.Now().Add(-1 * time.Hour).Unix(),
							},
				...
				Context("with another in-flight exclusion", func() {
					...
					It("should not mark the process group for removal", func() {
						Expect(
							getRemovedProcessGroupIDs(cluster),
						).To(ConsistOf([]fdbv1beta2.ProcessGroupID{secondProcessGroup.ProcessGroupID}))
					})
					...
					When("max concurrent replacements is set to two", func() {
						...
						It("should mark the process group for removal", func() {
							Expect(
								getRemovedProcessGroupIDs(cluster),
							).To(ConsistOf([]fdbv1beta2.ProcessGroupID{processGroup.ProcessGroupID, secondProcessGroup.ProcessGroupID}))
						})
					})
				...
				Context("with maintenance mode enabled", func() {
					BeforeEach(func() {
						...
						Expect(err).NotTo(HaveOccurred())
						Expect(
							adminClient.SetMaintenanceZone(
								"operator-test-1-"+string(processGroup.ProcessGroupID),
								0,
							),
						).NotTo(HaveOccurred())
					})
			...
			When(
				"a process is marked for removal and has the ProcessIsMarkedAsExcluded condition",
				func() {
					BeforeEach(func() {
						processGroup.ProcessGroupConditions = append(
							processGroup.ProcessGroupConditions,
		...
		When("fault domain replacements are enabled", func() {
			...
			Context("with a process that has been missing for a long time", func() {
				...
				Context("with multiple failed processes", func() {
					...
					When("those failed processes are on the same fault domain", func() {
						BeforeEach(func() {
							...
							secondProcessGroup.FaultDomain = fdbv1beta2.FaultDomain(
								cluster.Name + "-" + string(processGroup.ProcessGroupID),
							)
						})
				...
				When("the process group has no addresses", func() {
					BeforeEach(func() {
						processGroup.Addresses = nil

						_ = cluster.Status.ProcessGroups
					})
...
func getRemovedProcessGroupIDs(
