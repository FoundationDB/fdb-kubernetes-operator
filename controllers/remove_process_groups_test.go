...
var _ = Describe("remove_process_groups", func() {
	...
	Describe("hasUnverifiableExcludedRemoval", func() {
		var cluster *fdbv1beta2.FoundationDBCluster
		...
		When("FDB reports no processes and a removal is flagged excluded", func() {
			BeforeEach(func() {
				cluster.Status.ProcessGroups[0].MarkForRemoval()
				cluster.Status.ProcessGroups[0].ExclusionTimestamp = &metav1.Time{
					Time: time.Now(),
				}
			})

			It("blocks removal because exclusion state is unverifiable", func() {
				Expect(hasUnverifiableExcludedRemoval(cluster, status)).To(BeTrue())
			})
		})

		When("FDB reports no processes but no removal is flagged excluded", func() {
			BeforeEach(func() {
				cluster.Status.ProcessGroups[0].MarkForRemoval()
			})

			It("does not block — there is no stale flag to act on", func() {
				Expect(hasUnverifiableExcludedRemoval(cluster, status)).To(BeFalse())
			})
		})

		When("FDB reports processes for the cluster's DC", func() {
			BeforeEach(func() {
				cluster.Status.ProcessGroups[0].MarkForRemoval()
				cluster.Status.ProcessGroups[0].ExclusionTimestamp = &metav1.Time{
					Time: time.Now(),
			...
			It("does not block — exclusion state is verifiable", func() {
				Expect(hasUnverifiableExcludedRemoval(cluster, status)).To(BeFalse())
			})
		})
		...
		When("status is nil", func() {
			It("does not block", func() {
				Expect(hasUnverifiableExcludedRemoval(cluster, nil)).To(BeFalse())
			})
		})
	})
})
