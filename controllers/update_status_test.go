	...
	Context("validate process group", func() {
		...
		When("a process group is fine", func() {
			It("should not get any condition assigned", func() {
				err := validateProcessGroups(context.TODO(), clusterReconciler, cluster, processMap, configMap, allPvcs, logger)
				Expect(err).NotTo(HaveOccurred())
		...
		When("a process group is not reporting to the cluster", func() {
			...
			When("no processes are provided in the process map", func() {
				It("should not get a condition assigned", func() {
					err := validateProcessGroups(context.TODO(), clusterReconciler, cluster, map[fdbv1beta2.ProcessGroupID][]fdbv1beta2.FoundationDBStatusProcessInfo{}, configMap, allPvcs, logger)
					Expect(err).NotTo(HaveOccurred())
