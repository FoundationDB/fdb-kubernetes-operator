...
| deletePodsWhenNodeSelectorChanges | DeletePodsWhenNodeSelectorChanges defines if a pod should be deleted when the node selector changes. This could be usedfull in combination with managed storage with retain policy to achieve light weight node replacements. | *bool | false |
