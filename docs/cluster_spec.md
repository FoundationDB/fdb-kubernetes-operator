...
| faultDomainBasedReplacements | FaultDomainBasedReplacements controls whether automatic replacements are targeting all failed process groups in a fault domain or only specific Process Groups. If this setting is enabled MaxConcurrentReplacements will provide the information in how many fault domains failed process groups can be replaced. The default is true. | *bool | false |
