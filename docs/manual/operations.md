...
Maintenance mode is supported for both `Zone` and `ProcessGroup` deletion modes. Note that in `ProcessGroup` deletion
mode, where pods are recreated one at a time, maintenance will be set for the pod's zone (which may contain other
pods). 
